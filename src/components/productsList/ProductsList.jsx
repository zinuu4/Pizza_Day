...
const ProductsList = () => {
  ...
  const renderContentFunc = () => {
    if (term.length === 0) {
      return (
        <>
          <h1 ref={pizzaFor155UahRef} className="products__title">
            Pizza for 155 uah
          </h1>
          <ul className="products__list">
            {pizzaFor155Uah.map(
              ({ img, name, weight, price, descr, id, Additives }) => {
                return (
                  <ProductCard
                    additives={Additives}
                    key={id}
          ...
          <h1 ref={pizzaFor129UahRef} className="products__title">
            Pizza for 129 uah
          </h1>
          <ul className="products__list">
            {pizzaFor129Uah.map(
          ...
          <h1 ref={pizzaFor115UahRef} className="products__title">
            Pizza for 115 uah
          </h1>
          <ul className="products__list">
            {pizzaFor115Uah.map(
          ...
          <h1 ref={pizzaFor99UahRef} className="products__title">
            Pizza for 99 uah
          </h1>
          <ul className="products__list">
            {pizzaFor99Uah.map(
          ...
          <h1 ref={saucesRef} className="products__title">
            Sauces
          </h1>
          <ul className="products__list">
          ...
          <h1 ref={drinksRef} className="products__title">
            Drinks
          </h1>
          <ul className="products__list">
      ...
      if (visibleData.length >= 1) {
        return (
          <ul className="products__list">
            {visibleData.map(
              ({ img, name, weight, volume, price, descr, id }) => {
                return (
                  <ProductCard
                    key={id}
                    img={img}
        ...
        return (
          <div className="foundNothing">
            ...
            <p className="foundNothing__text">
              Unfortunately, we did not find such a dish with us. Try something
              else.
            </p>
          </div>
        );
