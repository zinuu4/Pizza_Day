...
const HistoryOfOrders = () => {
  ...
  const { getDataByDocument, loading, error } = useHttp();
    ...
    if (historyOfOrders.orders?.length >= 1) {
      return (
        <ul className="ordersHistory">
          {historyOfOrders.orders.map(
            ({ date, id, orderPrice, receivingMethod, time }) => {
              return (
                  ...
                  <div className="ordersHistory__info">
                    <div className="ordersHistory__fields-wrapper">
                      <div className="ordersHistory__id-wrapper">
                        ...
                        <div className="ordersHistory__info ordersHistory__id">
                          Order №: {id.substring(0, 12)}...
                        </div>
                      </div>

                      <div className="ordersHistory__info ordersHistory__date">
                        Date {date} at {time}
                      </div>
                    </div>

                    <div className="ordersHistory__fields-wrapper">
                      <div className="ordersHistory__info ordersHistory__price">
                        Order price: {orderPrice} ₴
                      </div>
                      <div className="ordersHistory__info ordersHistory__receiving">
                        Receiving: {receivingMethod}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      return (
        <div className="emptyProfile">
          ...
          <p className="emptyProfile__text">
  ...
  const content = !(error || loading) ? renderContentFunc : null;
