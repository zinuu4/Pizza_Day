...
const DeliveryModal = () => {
  ...
  const renderedRestaurants = cityRestaurants.map(
    ({ name, img, id, address, timeOpen, lat, lng }) => {
      const restaurantClass =
        name === chosenRestaurant.name
          ? "choose__restaurant selected-restaurant"
          : "choose__restaurant";
      const addressClass =
        name === chosenRestaurant.name
          ? "choose__restaurant-address selected-restaurant-address"
          : "choose__restaurant-address";
      return (
        <div
          onClick={() =>
            dispatch(
  ...
  const content = !(getDataError || getDataLoading)
    ? renderedRestaurants
    : null;
        ...
        >
          <div className="modal deliveryModal">
            <div className="deliveryModal__top">
              ...
              <button
                onClick={() => dispatch(setDeliveryModal(false))}
                className="btn-close"
              >
            ...
            <div className="deliveryModal__content">
              <div className="deliveryModal__content__left">
                <div className="deliveryModal__choose__btns">
                  <button className="deliveryModal__choose__btn">
                    Take-out
                  </button>
                </div>
                ...
                <select
                  value={chosenCity}
                  onChange={(e) => dispatch(setChosenCity(e.target.value))}
                  className="choose__city"
                >
                  {cities.map(({ id }) => {
                    return (
                      <option key={id} className="choose__city-option">
                        {id}
                      </option>
                    );
                  })}
              ...
              <div className="deliveryModal__content__right">
                ...
                <p className="deliveryModal__info">
